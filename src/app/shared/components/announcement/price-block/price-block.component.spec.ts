...
import { findNearestRoutePoint, findNearestRouteStop, formatRouteDistance, parseRoutePath, parseRouteStops, } from './transport-route.utils';

describe('PriceBlockComponent', () => {
    let component: PriceBlockComponent;
    let requestService: MockedObject<any>;

    beforeEach(() => {
        requestService = {
            requestData: vi.fn().mockName("RequestService.requestData")
        };
        component = new PriceBlockComponent({
            navigate: vi.fn().mockName("Router.navigate")
        } as any, { auth: false, user: {} } as any, {
            save: vi.fn().mockName("ChatUrlService.save")
        } as any, requestService, { run: (callback: () => void) => callback() } as any);
    });
    ...
    it('loads a selected bus route and exposes its proximity to the listing', () => {
        requestService.requestData.mockImplementation((url: string) => of(url === '/api/bus/' ? {
            stops: {
                forward: [
        ...
        expect(vi.mocked(requestService.requestData).mock.calls[0]).toEqual(['/api/bus/', 'POST', { id: '101' }]);
        ...
        expect(vi.mocked(requestService.requestData).mock.lastCall).toEqual([
            '/api/walking-route/',
            'POST',
            {
    ...
    it('shows stop markers only at a readable zoom level', () => {
        expect(component.showRouteStops).toBe(false);

        component.onMapBoundsChange({ event: { get: () => 14 } });
        expect(component.showRouteStops).toBe(true);
    ...
    it('does not draw a straight fallback when pedestrian routing fails', () => {
        requestService.requestData.mockImplementation((url: string) => url === '/api/bus/' ? of({
            scheme: {
                forward: '41.0000,69.0010 41.0100,69.0010',
                backward: '',
            },
    ...
    it('loads metro and marshrutka routes through the local transport endpoint', () => {
        ...
        expect(vi.mocked(requestService.requestData).mock.calls).toEqual([
            ['/api/bus/', 'POST', { id: '3' }],
            ['/api/walking-route/', 'POST', expect.any(Object)],
            ['/api/bus/', 'POST', { id: '170' }],
            ['/api/walking-route/', 'POST', expect.any(Object)],
        ]);
    });
