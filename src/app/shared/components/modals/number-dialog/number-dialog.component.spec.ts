...
describe('NumberDialogComponent', () => {
    ...
    beforeEach(() => {
        localStorage.clear();
        requestService = {
            requestData: vi.fn().mockReturnValue(of({ phone_number: '+998901234567' })),
        };
        component = new NumberDialogComponent(requestService as any);
        component.ruleForm.setValue({ phone_number: '90 123 45 67' });
        vi.spyOn(component, 'eventPipe').mockReturnValue(undefined);
    });
    ...
    it('stores local OTP only in development when backend marks it local', () => {
        ...
        requestService.requestData.mockReturnValue(of({ phone_number: '+998901234567', otp_code: '12345', local_otp: true }));
    ...
    it('does not store OTP without local_otp marker', () => {
        environment.production = false;
        requestService.requestData.mockReturnValue(of({ phone_number: '+998901234567', otp_code: '12345' }));
